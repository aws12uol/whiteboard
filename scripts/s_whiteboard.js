...
const { getSafeFilePath, uploadFileData, readFileData } = require("./utils");
...
module.exports = {
    ...
    saveToDB: function (wid) {
        if (config.backend.enableFileDatabase) {
            ...
            // upload to global DB after 30 sec
            setTimeout(function () {
                if (savedBoards[wid]) {
                    console.log(JSON.stringify(savedBoards[wid]));
                    uploadFileData({
                        updatedAt: new Date(),
                        fileName: wid,
                        data: JSON.stringify(savedBoards[wid]),
                    });
                }
            }, 1000 * 30);
        }
    ...
    loadStoredData: function (wid) {
        ...
        if (config.backend.enableFileDatabase) {
            ...
            if (fs.existsSync(filePath)) {
                ...
                // else {
                // read from global DB
                readFileData(wid).then((data) => {
                    if (data) {
                        savedBoards[wid] = JSON.parse(data.data);
                    }
                });
