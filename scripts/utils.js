...
const WhiteBoardSchema = require("./whiteBoardSchema");
...
const uploadFileData = async (data) => {
    return await WhiteBoardSchema.create(data).then((data) => {
        return data._id;
    });
};

const readFileData = async (wid) => {
    return await WhiteBoardSchema.findOne({ fileName: { $regex: wid } })
        .sort({ field: "asc", updatedAt: -1 })
        .then((data) => {
            console.info("JSON fetched from DB");
            return data;
        });
};
